...
import os
import random
from flask import Blueprint, request, abort, session, Response, json
...
- The bot will then send the following things in addition to other requirements for the endpoint in every POST request
    1. The BotID being used or the hash of the BotID
    2. The ciphertext from encrypting the BotID with the API key
        - The bot will not send the API key
- The server will then decrypt the ciphertext. If the resulting plaintext matches the BotID, then the client has verified
possession of the correct API key
"""

api = Blueprint('api', __name__, url_prefix='/api')
api.secret_key = os.urandom(64)
print(f'Connection Key: {base64.b64encode(api.secret_key)}')
...
def FlaskBotSessionOpen():
    ...
    if key == api.secret_key:
        ...
        botApiKey = str(base64.b64encode(os.urandom(64)))
        ...
        resp = Response(json.jsonify(BotID=botID, Key=botApiKey), status=200, mimetype='application/json')
